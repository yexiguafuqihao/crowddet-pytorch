...
class CrowdHuman(torch.utils.data.Dataset):
    ...
    def __init__(self, config, if_train, splitted_data = None):
        if if_train:
            self.training = True
        ...
        if splitted_data is not None and if_train == False:
            self.records =splitted_data
    ...
    def resize_img_by_short_and_max_size(self,
    ...
    def apply(self, inputs):
        ...
        # return self.merge_batch(inputs)
        hw_stat = np.stack([t[0].shape[:2] for t in inputs], axis=0)
        nr_info_dim = config.nr_info_dim
        batch_per_gpu = len(inputs)

        batch_image_height = np.max(hw_stat[:, 0])
        batch_image_width = np.max(hw_stat[:, 1])

        filter_box_size = config.batch_filter_box_size
        batch_resized_height, batch_resized_width = self.get_hw_by_short_size(
            batch_image_height, batch_image_width, self.short_size, self.max_size)

        batch_images = np.zeros(
            (batch_per_gpu, 3, batch_resized_height, batch_resized_width),
            dtype=np.float32)
        batch_gts = np.zeros(
            (batch_per_gpu, config.max_boxes_of_image, config.nr_box_dim),
            dtype=np.float32)
        batch_info = np.zeros((batch_per_gpu, config.nr_info_dim), dtype=np.float32)

        for i in range(batch_per_gpu):
         
            img, gt, im_info = inputs[i]

            padded_image, padded_gt = self.pad_image_and_boxes(
            ...
            resized_image, scale = self.resize_img_by_short_and_max_size(
                padded_image, self.short_size, self.max_size)
            ...
            batch_images[i] = resized_image.transpose(2, 0, 1).astype(
                np.float32)
        ...
        return batch_images, batch_gts, batch_info

    def collate_fn(self, inputs):

        images, boxes, im_info = self.apply(inputs)

        images = torch.tensor(images)
        boxes = torch.tensor(boxes)
        im_info = torch.tensor(im_info)

        return images, boxes, im_info
