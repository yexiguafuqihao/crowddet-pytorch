...
def restore_bbox(rois, deltas, unnormalize=True, config = None):
    ...
    m, c = deltas.shape[1], rois.shape[1]

    rois = rois.unsqueeze(1).repeat(1, m, 1).reshape(-1, c)
    deltas = deltas.reshape(-1, 4)
    pred_bbox = bbox_transform_inv_opr(rois, deltas)
    pred_bbox = pred_bbox.reshape(-1, m, 4)
...
def bbox_transform_opr(bbox, gt):
    ...
    target = torch.stack([target_dx, target_dy, target_dw, target_dh], dim = 1)
    return target
...
def box_overlap_ignore_opr(box, gt, ignore_label=-1):
    ...
    ioa = torch.where(
        ...
        inter / (area_box[:, None]),
        torch.zeros(1, dtype=inter.dtype, device=inter.device))
    gt_ignore_mask = gt[:, 4].eq(ignore_label).repeat(box.shape[0], 1).float()
    iou, ioa = iou * (1 - gt_ignore_mask), ioa * gt_ignore_mask
    return iou, ioa
