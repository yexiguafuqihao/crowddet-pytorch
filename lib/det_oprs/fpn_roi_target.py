...
def fpn_roi_target(rpn_rois, im_info, gt_boxes, fg_threshold=config.fg_threshold, top_k=1):
    ...
    return_labels = []
    return_bbox_targets = []
    sampling = True#fg_threshold <= 0.5
    # get per image proposals and gt_boxes
    train_batch_per_gpu = torch.unique(rpn_rois[:, 0]).shape[0]
    for bid in range(train_batch_per_gpu):
        ...
        all_rois = torch.cat([rpn_rois[batch_roi_inds], gt_rois], axis=0) if sampling  \
            else rpn_rois[batch_roi_inds]
        ...
        fg_mask = (max_overlaps >= fg_threshold) * (labels != config.ignore_label)
        ...
        fg_inds_mask = subsample_masks(fg_mask[:, 0], pos_max, True) if sampling else fg_mask[:, 0]
        neg_max = config.num_rois - fg_inds_mask.sum()
        bg_inds_mask = subsample_masks(bg_mask[:, 0], neg_max, True) if sampling else bg_mask[:, 0]
